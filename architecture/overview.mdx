...
- The `url` field in a Discord activity must be a valid streaming URL. The bot checks whether `track.uri` belongs to YouTube (`youtube.com` / `youtu.be`) or Twitch (`twitch.tv`); any other domain (e.g. SoundCloud, direct HTTP streams) falls back to `https://www.youtube.com`.
